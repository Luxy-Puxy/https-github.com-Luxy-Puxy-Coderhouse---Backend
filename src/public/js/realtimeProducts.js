...
formPost.addEventListener("submit", async (event) => {
    ...
    bothStatus.forEach((option) => {
        if (option.checked) {
            option.value === "true" ? (status = true) : (status = false);
        }
    });
    ...
    if (!title.value || !description.value || !category.value || !code.value || !thumbnail.value) {
        return alert("Please complete all fields.");
    }
    if (stock.value === undefined || stock.value === null || !Number(stock.value)) {
        return alert("Stock must be a number.");
    }
    if (price.value === undefined || price.value === null || !Number(price.value)) {
        return alert("Price must be a number.");
    }
    if (!/(jpg|gif|png|JPG|GIF|PNG|JPEG|jpeg)$/.test(thumbnail.value)) {
        thumbnail.value = thumbnailPlaceholder;
    }
    ...
    try {
        ...
        if (!ok) {
            return alert(postProductResponse.response);
        }
...
formDelete.addEventListener("submit", async (event) => {
    ...
    if (!id) {
        return alert("Please provide an ID.");
    }

    try {
        const response = await fetch(`/realtimeproducts/${id}`, {
            method: "DELETE",
        });
